...
Setup_aria2(){
	...
	if [[ ! ${update_dl} == "update" ]]; then
		echo -e "${Info} 请输入当前 Aria2 安装程序版本号，格式如：[ 1.35.0 ]"
		read -e -p "默认为最新版本号:" aria2_ver
		[[ -z "${aria2_ver}" ]] && aria2_ver=$(wget -qO- https://api.github.com/repos/q3aql/aria2-static-builds/releases | grep -o '"tag_name": ".*"' |head -n 1| sed 's/"//g;s/v//g' | sed 's/tag_name: //g')
		if [[ -z ${aria2_ver} ]]; then
			read -e -p "自动获取最新版本号失败，请手动输入，格式如：[ 1.35.0 ]:" aria2_ver_s && aria2_ver=${aria2_ver_s}
		fi
	fi
	Aria2_Name="aria2-${aria2_ver}-linux-gnu-${bit}-build1"
	echo -e "文件名称： ${Aria2_Name}"
	[[ ! -s "${Aria2_Name}.tar.bz2" ]] && echo -e "${Error} Aria2 主程序压缩包不存在！请确认/root目录下有安装压缩包！获取地址可在帮助中查看。" && exit 1
	tar jxvf "${Aria2_Name}.tar.bz2"
    [[ ! -e "${Aria2_Name}" ]] && echo -e "${Error} Aria2 解压失败 !" && rm -rf "${Aria2_Name}" && exit 1
    [[ ${update_dl} == "update" ]] && rm -rf "${Folder}"
    mv "${Aria2_Name}" "${Folder}"
    [[ ! -e "${Folder}" ]] && echo -e "${Error} Aria2 文件夹重命名失败 !" && rm -rf "${Aria2_Name}" && exit 1
    rm -rf "${Aria2_Name}.tar.bz2"
    cd "${Folder}"
    make install
    [[ ! -e ${aria2c} ]] && echo -e "${Error} Aria2 主程序安装失败！" && rm -rf "${Folder}" && exit 1
...
Download_aria2_conf(){
    ...
    wget -N "https://raw.githubusercontent.com/MeowPerth/Aria2-Install-Linux/master/root/.aria2/aria2.conf"
    [[ ! -s "aria2.conf" ]] && echo -e "${Error} Aria2 配置文件下载失败 !" && rm -rf "${file}" && exit 1
    wget -N "https://raw.githubusercontent.com/MeowPerth/Aria2-Install-Linux/master/root/.aria2/dht.dat"
    [[ ! -s "dht.dat" ]] && echo -e "${Error} Aria2 DHT（IPv4）文件下载失败 !" && rm -rf "${file}" && exit 1
    wget -N "https://raw.githubusercontent.com/MeowPerth/Aria2-Install-Linux/master/root/.aria2/dht6.dat"
    [[ ! -s "dht6.dat" ]] && echo -e "${Error} Aria2 DHT（IPv6）文件下载失败 !" && rm -rf "${file}" && exit 1
    touch "${file}/aria2.session"
    [[ ! -e "/root/.aria2/aria2.session" ]] && echo -e "${Error} Aria2 任务文件创建失败 !" && rm -rf "${file}" && rm -rf "/root/${zip_name}" && exit 1
    sed -i 's/^rpc-secret=P3TERX/rpc-secret='$(date +%s%N | md5sum | head -c 20)'/g' ${aria2_conf}
    echo -e "${Info} Aria2 配置下载完成！"
}
Download_aria2_Service(){
    if [[ ${release} = "centos" ]]; then
        if ! wget https://raw.githubusercontent.com/MeowPerth/Aria2-Install-Linux/master/etc/init.d/aria2_centos -O /etc/init.d/aria2; then
        ...
        if ! wget https://raw.githubusercontent.com/MeowPerth/Aria2-Install-Linux/master/etc/init.d/aria2_debian -O /etc/init.d/aria2; then
            echo -e "${Error} Aria2服务 管理脚本下载失败 !" && exit 1
        fi
        chmod +x /etc/init.d/aria2
...
Setup_aria2_conf_and_Service(){
	cd "/root"
	# wget https://github.com/MeowPerth/Aria2-Install-Linux/archive/master.zip -O /root/Aria2-Install-Linux-master.zip
	[[ ! -s "Aria2-Install-Linux-master.zip" ]] && echo -e "${Error} Aria2 配置文件压缩包不存在！获取地址可在帮助中查看。" && exit 1
	unzip "Aria2-Install-Linux-master.zip"
	zip_name="Aria2-Install-Linux-master"
	mkdir -p "${file}" && mv -vt "${file}" "${zip_name}/root/.aria2/aria2.conf" "${zip_name}/root/.aria2/dht.dat" "${zip_name}/root/.aria2/dht6.dat"
    [[ ! -s "/root/.aria2/aria2.conf" ]] && echo -e "${Error} Aria2 配置文件安装失败 !" && rm -rf "${file}" && rm -rf "/root/${zip_name}" && exit 1
    [[ ! -s "/root/.aria2/dht.dat" ]] && echo -e "${Error} Aria2 DHT（IPv4）文件安装失败 !" && rm -rf "${file}" && rm -rf "/root/${zip_name}" && exit 1
    [[ ! -s "/root/.aria2/dht6.dat" ]] && echo -e "${Error} Aria2 DHT（IPv6）文件安装失败 !" && rm -rf "${file}" && rm -rf "/root/${zip_name}" && exit 1
    sed -i 's/^rpc-secret=P3TERX/rpc-secret='$(date +%s%N | md5sum | head -c 20)'/g' ${aria2_conf}
    touch "${file}/aria2.session"
    [[ ! -e "/root/.aria2/aria2.session" ]] && echo -e "${Error} Aria2 任务文件安装失败 !" && rm -rf "${file}" && rm -rf "/root/${zip_name}" && exit 1
    echo -e "${Info} Aria2 配置安装完成！"
    
    if [[ ${release} = "centos" ]]; then
    	mv -v "${zip_name}/etc/init.d/aria2_centos" "/etc/init.d/aria2"
        [[ ! -s "/etc/init.d/aria2" ]] && echo -e "${Error} Aria2服务 管理脚本下载失败 !" && exit 1
        chmod +x /etc/init.d/aria2
        chkconfig --add aria2
        chkconfig aria2 on
    else
    	mv -v "${zip_name}/etc/init.d/aria2_debian" "/etc/init.d/aria2"
        [[ ! -s "/etc/init.d/aria2" ]] && echo -e "${Error} Aria2服务 管理脚本下载失败 !" && exit 1
        chmod +x /etc/init.d/aria2
        update-rc.d -f aria2 defaults
    fi
    echo -e "${Info} Aria2服务 管理脚本安装完成 !"
    rm -rf "${zip_name}.zip" && rm -rf "/root/${zip_name}"
}
...
Install_aria2_local(){
    ...
    echo -e "${Info} 开始安装/配置 依赖..."
    Installation_dependency
    echo -e "${Info} 开始安装 本地主程序..."
    Setup_aria2
    echo -e "${Info} 开始安装 Aria2 配置及服务脚本(init)..."
    Setup_aria2_conf_and_Service
    Read_config
    aria2_RPC_port=${aria2_port}
    echo -e "${Info} 开始设置 iptables防火墙..."
...
Update_aria2_local(){
	...
	aria2_now_ver=$(${aria2c} -v|head -n 1|awk '{print $3}')
    [[ -z ${aria2_now_ver} ]] && echo -e "${Error} Aria2 当前版本获取失败 !" && exit 1
    echo -e "${Info} 请输入当前 Aria2 更新程序版本号，格式如：[ 1.35.0 ]"
	read -e -p "默认为最新版本号:" aria2_ver
	[[ -z "${aria2_ver}" ]] && aria2_ver=$(wget -qO- https://api.github.com/repos/q3aql/aria2-static-builds/releases | grep -o '"tag_name": ".*"' |head -n 1| sed 's/"//g;s/v//g' | sed 's/tag_name: //g')
	if [[ -z ${aria2_ver} ]]; then
		read -e -p "自动获取最新版本号失败，请手动输入，格式如：[ 1.35.0 ]:" aria2_ver_s && aria2_ver=${aria2_ver_s}
	fi
    if [[ "${aria2_now_ver}" != "${aria2_ver}" ]]; then
        echo -e "${Info} 发现 Aria2 已有新版本 [ ${aria2_ver} ](当前版本：${aria2_now_ver})"
        read -e -p "是否更新(会中断当前下载任务，请注意) ? [Y/n] :" yn
        [[ -z "${yn}" ]] && yn="y"
        if [[ $yn == [Yy] ]]; then
            check_pid
        ...
        echo -e "${Info} 当前 Aria2 已是最新版本 [ ${aria2_ver} ]" && exit 1
    fi
}
...
Get_aria2_url(){
    ...
    aria2_new_ver=$(wget -qO- https://api.github.com/repos/q3aql/aria2-static-builds/releases | grep -o '"tag_name": ".*"' |head -n 1| sed 's/"//g;s/v//g' | sed 's/tag_name: //g')
    if [[ -z ${aria2_new_ver} ]]; then
		var_msg="Aria2 安装程序下载地址：
${Red_font_prefix}https://github.com/q3aql/aria2-static-builds/releases${Font_color_suffix}
当前系统适用版本：${Green_font_prefix}linux-gnu-${bit}-build1${Font_color_suffix}
文件名称示例：${Green_font_prefix}aria2-1.35.0-linux-gnu-${bit}-build1.tar.bz2${Font_color_suffix}"
    else
		var_msg="Aria2 安装程序下载路径：
${Red_font_prefix}https://github.com/q3aql/aria2-static-builds/releases/download/v${aria2_new_ver}/aria2-${aria2_new_ver}-linux-gnu-${bit}-build1.tar.bz2${Font_color_suffix}"
	fi
}
Readme(){
	echo && echo -e "
==========================================================
==========================================================
	${Green_font_prefix}-- 说明 --${Font_color_suffix}"
	echo -e "此脚本是基于 P3TERX 的脚本进行修改而成的，如有不足之处还望指出。
	
1. 脚本中提供了本地安装功能，旨在解决大陆地区从Git上拉取代码缓慢或是无法拉取文件,导致无法正常安装的问题。
PS：进行本地安装，请先确认 Aria2 主程序及配置文件的压缩包在${Red_font_prefix}/root${Font_color_suffix}目录下。
...
=  2.完善 部分脚本代码。
=========================================================="
	echo -e "	${Green_font_prefix}-- 帮助 --${Font_color_suffix}"
	echo -e "${var_msg}
Aria2 配置&服务文件下载路径：
${Red_font_prefix}https://github.com/MeowPerth/Aria2-Install-Linux/archive/master.zip -O /root/Aria2-Install-Linux-master.zip${Font_color_suffix}
==========================================================
=========================================================="
}
Install_aria2_choose(){
    echo && echo -e " ${Green_font_prefix}请选择安装方式 ：${Font_color_suffix}
————————————
 ${Green_font_prefix} 0.${Font_color_suffix} 在线安装 Aria2
————————————
 ${Green_font_prefix} 1.${Font_color_suffix} 本地安装 Aria2（需要有安装 及 配置&服务 文件压缩包）
————————————"
	echo -e " ${Red_font_prefix}[默认为：0]${Font_color_suffix} "
	read -e -p " 请输入数字 [0-1]：" num_install
	[[ -z "${num_install}" ]] && num_install="0"
	case "$num_install" in
    	0)
    	echo -e "${Info} 开始在线安装 Aria2"
    	Install_choose="install_online"
    	Install_aria2_online
    	;;
		1)
    	echo -e "${Info} 开始本地安装 Aria2"
    	Install_choose="install_local"
    	Install_aria2_local
    	;;
    	*)
    	echo "请输入正确数字 [0-1]"
...
Update_aria2_choose(){
    echo && echo -e " ${Green_font_prefix}请选择更新方式 ：${Font_color_suffix}
————————————
 ${Green_font_prefix} 0.${Font_color_suffix} 在线更新 Aria2
————————————
 ${Green_font_prefix} 1.${Font_color_suffix} 本地更新 Aria2（需要有更新 文件压缩包）
————————————"
	echo -e " ${Red_font_prefix}[默认为：0]${Font_color_suffix} "
	read -e -p " 请输入数字 [0-1]：" num_install
	[[ -z "${num_install}" ]] && num_install="0"
	case "$num_install" in
    	0)
    	echo -e "${Info} 开始在线更新 Aria2"
    	Install_choose="Update_aria2_online"
    	Update_aria2_online
    	;;
		1)
    	echo -e "${Info} 开始本地更新 Aria2"
    	Install_choose="Update_aria2_local"
    	Update_aria2_local
    	;;
    	*)
    	echo "请输入正确数字 [0-1]"
...
if [[ -e ${aria2c} ]]; then
    ...
    if [[ ! -z "${PID}" ]]; then
        ...
        echo -e " 当前版本：${Red_font_prefix}v$(${aria2c} -v|head -n 1|awk '{print $3}')"${Font_color_suffix}
    else
